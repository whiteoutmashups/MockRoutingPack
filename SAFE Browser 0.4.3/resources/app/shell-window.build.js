...
class DownloadsNavbarBtn {
    ...
    if (this.isDropdownOpen) {
      var downloadEls = activeDownloads.map(d => {
        ...
        if (d.state == 'completed') {
          ...
          ctrlsEl = yo`<div class="td-item-ctrls">
            ${ d.isPaused ? yo`<a href="#" onclick=${ e => this.onResume(e, d) }>resume</a>` : yo`<a href="#" onclick=${ e => this.onPause(e, d) }>pause</a>` }
            |
        ...
        return yo`<div class="td-item">
          ...
          ${ d.state == 'progressing' ? yo`<div class="td-item-progress"><progress value=${ d.receivedBytes } max=${ d.totalBytes }></progress></div>` : '' }
          ${ ctrlsEl }
        </div>`;
      });
  ...
  doAnimation() {
    Array.from(document.querySelectorAll('.toolbar-downloads-btn')).forEach(el => el.animate([{ transform: 'scale(1.0)', color: 'inherit' }, { transform: 'scale(1.5)', color: '#06c' }, { transform: 'scale(1.0)', color: 'inherit' }], { duration: 300 }));
  }
  ...
  onShow(e, download) {
    e.preventDefault();
    e.stopPropagation();
    beakerDownloads.showInFolder(download.id).catch(err => {
  ...
  onOpen(e, download) {
    e.preventDefault();
    e.stopPropagation();
    beakerDownloads.open(download.id).catch(err => {
      download.fileNotFound = true;
      this.updateActives();
    });
  }
  ...
  onOpenDownloads(e) {
    e.preventDefault();
    e.stopPropagation();
    setActive(create('beaker:downloads'));
    this.isDropdownOpen = false;
    this.updateActives();
  }
}

class SitePermsNavbarBtn {
  constructor() {
    this.isDropdownOpen = false;
  ...
  render() {
    if (!this.protocolDescription) return;
    ...
    var icon = 'window';
    var label = '';
    if (this.protocolDescription.label == 'HTTPS') icon = 'lock';
...
function updateLocation(page) {
  ...
  if (!isAddrElFocused || !addrEl.value) {
    // only update if not focused or empty, so we dont mess up what the user is doing
...
function render(id, page) {
  ...
  var reloadBtn = isLoading ? yo`<button class="toolbar-btn nav-cancel-btn" onclick=${ onClickCancel }>
        <span class="icon icon-cancel"></span>
      </button>` : yo`<button class="toolbar-btn nav-reload-btn" onclick=${ onClickReload }>
        <span class="icon icon-ccw"></span>
      </button>`;

  // render safe btn
  var safeBtn = isSafe ? yo`<button class="toolbar-btn safe-btn-safe" onclick=${ onClickToggleSafe }>
        <span class="icon icon-rocket"></span>
      </button>` : yo`<button class="toolbar-btn safe-btn-not-safe" onclick=${ onClickToggleSafe }>
  ...
  // inpage finder ctrl
  var inpageFinder = page && page.isInpageFinding ? yo`<input
            type="text"
  ...
  if (autocompleteResults) {
    autocompleteDropdown = yo`
      ...
      if (r.search) contentColumn = yo`<span class="result-search">${ r.search }</span>`;else {
        ...
        if (r.urlDecorated) contentColumn.innerHTML = r.urlDecorated; // use innerHTML so our decoration can show
        else contentColumn.textContent = r.url;
      }
      var titleColumn = yo`<span class="result-title"></span>`;
      if (r.titleDecorated) titleColumn.innerHTML = r.titleDecorated; // use innerHTML so our decoration can show
      else titleColumn.textContent = r.title;
...
function handleAutocompleteSearch(results) {
  ...
  if (isProbablyUrl && !v.includes('://') && !(v.startsWith('beaker:') || v.startsWith('ipfs:/'))) {
    if (isHashRegex.test(v)) vWithProtocol = 'dat://' + v;else {
  ...
  if (isProbablyUrl) autocompleteResults = [gotoResult, searchResult];else autocompleteResults = [searchResult];
...
function getAutocompleteSelection(i) {
  if (typeof i !== 'number') i = autocompleteCurrentSelection;
  if (autocompleteResults && autocompleteResults[i]) return autocompleteResults[i];
...
function joinSegments(segments) {
  ...
  for (var segment of segments) {
    ...
    if (isBold) str += '<strong>' + segment + '</strong>';else str += segment;
    isBold = !isBold;
  }
  return str;
}
...
function getEventPage(e) {
  for (var i = 0; i < e.path.length; i++) if (e.path[i].dataset && e.path[i].dataset.id) return getById(e.path[i].dataset.id);
}

function onClickBack(e) {
  var page = getEventPage(e);
  if (page && page.canGoBack()) page.goBack();
}

function onClickForward(e) {
  var page = getEventPage(e);
  if (page && page.canGoForward()) page.goForward();
}
...
function onClickViewFiles(e) {
  ...
  if (page) {
    if (e.metaKey || e.ctrlKey) // popup
      setActive(create('view-' + page.getURL()));else page.loadURL('view-' + page.getURL()); // goto
  }
}

function onClickZoom(e) {
  const { Menu, MenuItem } = electron.remote;
  var menu = Menu.buildFromTemplate([{ label: 'Reset Zoom', click: () => zoomReset(getActive()) }, { label: 'Zoom In', click: () => zoomIn(getActive()) }, { label: 'Zoom Out', click: () => zoomOut(getActive()) }]);
  menu.popup(electron.remote.getCurrentWindow());
}

function onFocusLocation(e) {
  var page = getEventPage(e);
  if (page) page.navbarEl.querySelector('.nav-location-input').select();
}
...
function onInputFind(e) {
  ...
  var page = getEventPage(e);
  if (page) {
    if (str) page.findInPage(str);else page.stopFindInPage('clearSelection');
  }
}
...
function onKeydownFind(e) {
  ...
  if (e.keyCode == KEYCODE_ENTER) {
    ...
    let page = getEventPage(e);
    if (page) {
      if (str) page.findInPage(str, { findNext: true, forward: !backwards });else page.stopFindInPage('clearSelection');
    }
  }
}
...
function setZoomFromSitedata(page) {
...
function urlToData(url, width, height, cb) {
  var img = new Image();
...
function create(opts) {
  ...
  if (opts.isPinned) pages.splice(indexOfLastPinnedTab(), 0, page);else pages.push(page)
  ...
  ;[['getURL', ''], ['getTitle', ''], ['goBack'], ['canGoBack'], ['goForward'], ['canGoForward'], ['reload'], ['reloadIgnoringCache'], ['stop'], ['isDevToolsOpened', false], ['openDevTools'], ['closeDevTools'], ['send'], ['findInPage'], ['stopFindInPage']].forEach(methodSpec => {
    var name = methodSpec[0];
    var defaultReturn = methodSpec[1];
    page[name] = (...args) => {
      if (page.isWebviewReady) return page.webviewEl[name].apply(page.webviewEl, args);
...
const handleSafeStoreChange = _.debounce(handleStoreChange, 200);
...
function indexOfLastPinnedTab() {
  var index = 0;
  for (index; index < pages.length; index++) if (!pages[index].isPinned) break;
  return index;
}
...
function getByWebContents(webContents) {
  for (var i = 0; i < pages.length; i++) {
    if (pages[i].webviewEl && pages[i].webviewEl.getWebContents() == webContents) return pages[i];
  }
  return null;
}
...
function loadPinnedFromDB() {
  return beakerBrowser.getSetting('pinned-tabs').then(json => {
    try {
      JSON.parse(json).forEach(url => create({ url, isPinned: true }));
    } catch (e) {}
  });
}
...
function onDomReady(e) {
  ...
  if (page) {
    ...
    setZoomFromSitedata(page);
  }
}

function onNewWindow(e) {
  var page = create(e.url);
  if (e.disposition == 'foreground-tab') setActive(page);
}
...
function onDidStartLoading(e) {
  ...
  if (page) {
    ...
    update(page);
    hideInpageFind(page);
    if (page.isActive) setIsLoading(true);
  }
}
...
function onDidStopLoading(e) {
  ...
  if (page) {
    ...
    if (scheme == 'http:' || scheme == 'https:') page.protocolDescription = { label: scheme.slice(0, -1).toUpperCase() };else page.protocolDescription = beakerBrowser.getProtocolDescription(scheme);
...
function onDidGetResponseDetails(e) {
  if (e.resourceType != 'mainFrame') return;
...
function onPageFaviconUpdated(e) {
  if (e.favicons && e.favicons[0]) {
    ...
    urlToData(e.favicons[0], 16, 16, (err, dataUrl) => {
      if (dataUrl) beakerSitedata.set(page.getURL(), 'favicon', dataUrl);
    });
  }
}
...
function createWebviewEl(id, url) {
  ...
  el.dataset.id = id;
  el.setAttribute('preload', 'file://' + path.join(electron.remote.app.getAppPath(), 'webview-preload.build.js'));
...
function drawTab(page) {
  ...
  if (page.isLoading()) {
    ...
    // fallbacks
    else if (page.getURL().startsWith('beaker:')) favicon = yo`<img src="beaker-favicon:beaker">`;else // fallback to a potentially-cached icon
...
function repositionTabs(e) {
  ...
  allPages.forEach(p => {
    ...
    if (p.isPinned) availableWidth -= MIN_TAB_WIDTH;else numUnpinnedTabs++;
  });
...
function onClickCloseTabsToTheRight(page) {
  return () => {
    ...
    for (var i = ps.length - 1; i > index; i--) remove(ps[i]);
  };
}
...
function onContextMenuTab(page) {
  ...
  return e => {
    var menu = Menu.buildFromTemplate([{ label: 'New Tab', click: onClickNew }, { type: 'separator' }, { label: 'Duplicate', click: onClickDuplicate(page) }, { label: page.isPinned ? 'Unpin Tab' : 'Pin Tab', click: onClickPin(page) }, { type: 'separator' }, { label: 'Close Tab', click: onClickTabClose(page) }, { label: 'Close Other Tabs', click: onClickCloseOtherTabs(page) }, { label: 'Close Tabs to the Right', click: onClickCloseTabsToTheRight(page) }]);
    menu.popup(electron.remote.getCurrentWindow());
  };
}
...
function permsPrompt (reqId, webContentsId, permission) {
  const respond = decision => electron.ipcRenderer.send('permission-response', reqId, decision);

  // look up the page, deny if failed
  var page = getByWebContents(electron.remote.webContents.fromId(webContentsId));
  ...
  if (!permDesc) return respond(false);
    ...
    render: ({ rerender, onClose }) => {
      return yo`<div>
        ...
        <span class="promptbar-btns">
          <a class="btn" onclick=${ () => {
        respond(true);onClose();
      } }>Allow</a>
          <a onclick=${ () => {
        respond(false);onClose();
      } }>Don't Allow</a>
        </span>
        <a class="promptbar-close icon icon-cancel-squared" onclick=${ () => {
        respond(false);onClose();
      } }></a>
      </div>`;
    },
...
function setup$5() {
  electron.ipcRenderer.on('safeStore-updated', (e, type) => {
    return handleSafeStoreChange();
  });

  electron.ipcRenderer.on('command', function (event, type, arg1, arg2, arg3) {
    var page = getActive();
    switch (type) {
      case 'file:new-tab':
        var page = create(arg1);
        setActive(page);
        focusLocation(page);
        return;
      case 'file:open-location':
        return focusLocation(page);
      case 'file:close-tab':
        return remove(page);
      case 'file:reopen-closed-tab':
        return reopenLastRemoved();
      case 'edit:find':
        return showInpageFind(page);
      case 'view:reload':
        return page.reload();
      case 'view:hard-reload':
        return page.reloadIgnoringCache();
      case 'view:zoom-in':
        return zoomIn(page);
      case 'view:zoom-out':
        return zoomOut(page);
      case 'view:zoom-reset':
        return zoomReset(page);
      case 'view:toggle-dev-tools':
        return page.isDevToolsOpened() ? page.closeDevTools() : page.openDevTools();
      case 'history:back':
        return page.goBack();
      case 'history:forward':
        return page.goForward();
      case 'window:toggle-safe-mode':
        return toggleSafe();
      case 'window:disable-web-security':
        return toggleWebSecurity();
      case 'window:next-tab':
        return changeActiveBy(1);
      case 'window:prev-tab':
        return changeActiveBy(-1);
      case 'set-tab':
        return changeActiveTo(arg1);
      case 'load-pinned-tabs':
        return loadPinnedFromDB();
      case 'perms:prompt':
        return permsPrompt(arg1, arg2, arg3);
    }
  });
}
...
function setup$6() {
  ...
  window.addEventListener('mousewheel', e => {
    if (isTouching) {
      ...
      if (Math.abs(vertical) > Math.abs(horizontal)) hnorm = 0; // ignore if there's more vertical motion than horizontal
      else if (horizontal < 0 && !canGoBack() || horizontal > 0 && !canGoForward()) hnorm = horizontal = 0; // ignore if the navigation isnt possible in that direction
        else hnorm = horizontal / SWIPE_TRIGGER_DIST;
      ...
      // highlight 
      if (shouldGoBack()) leftSwipeArrowEl.classList.add('highlight');else leftSwipeArrowEl.classList.remove('highlight');
      if (shouldGoForward()) rightSwipeArrowEl.classList.add('highlight');else rightSwipeArrowEl.classList.remove('highlight');
    }
  });
  ...
  electron.ipcRenderer.on('window-event', function (event, type) {
...
},{"electron":undefined,"emit-stream":3,"events":undefined,"lodash":8,"path":undefined,"pauls-electron-rpc":12,"pretty-bytes":15,"url":undefined,"yo-yo":17}],2:[function(require,module,exports){
...
},{"events":undefined,"through":16}],4:[function(require,module,exports){
(function (global){
...
},{"./lib/export-api":13,"./lib/import-api":14}],13:[function(require,module,exports){
(function (Buffer){
...
},{"../../../../node_modules/is-buffer/index.js":20,"electron":undefined,"events":undefined,"stream":undefined}],14:[function(require,module,exports){
const EventEmitter = require('events')
...
},{"electron":undefined,"events":undefined,"stream":undefined}],15:[function(require,module,exports){
'use strict';
...
},{"number-is-nan":10}],16:[function(require,module,exports){
(function (process){
...
},{"_process":21,"stream":undefined}],17:[function(require,module,exports){
...
},{"./update-events.js":18,"bel":2,"morphdom":9}],18:[function(require,module,exports){
module.exports = [
